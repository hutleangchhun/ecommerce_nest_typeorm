...
const testRoleBasedAuth = async () => {
  ...
  for (const testUser of testUsers) {
    try {
      ...
      if (loginResult.access_token) {
        ...
        console.log(`✅ ${testUser.role.toUpperCase()}: Logged in successfully`);
      }
    } catch (error) {
  ...
  const permissionTests = [
    ...
    { endpoint: '/api/v1/categories', method: 'POST', admin: true, sales: true, customer: false },
    { endpoint: '/api/v1/products', method: 'GET', admin: true, sales: true, customer: true },
    { endpoint: '/api/v1/products', method: 'POST', admin: true, sales: true, customer: false },
  ...
  for (const test of permissionTests) {
    ...
    for (const role of ['admin', 'sales', 'customer']) {
      ...
      try {
        ...
        } else if (!expected && result === 403) {
          console.log(`  ✅ ${role.toUpperCase()}: Access denied (${result})`);
        } else if (!expected && result === 401) {
          console.log(`  ✅ ${role.toUpperCase()}: Unauthorized (${result})`);
        } else {
          console.log(`  ⚠️  ${role.toUpperCase()}: Unexpected result (${result})`);
        }
      } catch (error) {
...
// Start testing after a delay to ensure server is ready
setTimeout(() => {
  testRoleBasedAuth();
}, 2000);

console.log('⏳ Starting role-based authentication tests in 2 seconds...');
console.log('⚠️  Make sure the server is running with: npm run start:dev');
console.log('⚠️  Make sure the database is reset with: ./reset-db.sh\n');